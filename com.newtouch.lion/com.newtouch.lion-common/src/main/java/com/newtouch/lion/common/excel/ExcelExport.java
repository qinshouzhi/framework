...
public class ExcelExport<T> {
	...
	public void export(String title, String[] headers, Collection<T> dataset,
			OutputStream out) {
		this.export(title, headers, dataset, out,
				DateUtil.FORMAT_DATETIME_YYYY_MM_DD_HH_MM_SS);
	}

	/**
	 * 根据标题、标题栏、数据集、输出流、日期格式导出Excel文件 字体默认为微软雅黑
	 * @param title         表格标题
	 * @param headers       标题栏
	 * @param dataset       数据
	 * @param out   		输出流
	 ...
	 */
	public void export(String title, String[] headers, Collection<T> dataset,
			OutputStream out, String dataPattern) {
		// 声明一个工作薄
		...
		// 设置注释内容
		comment.setString(new HSSFRichTextString("注册"));
		// 设置注释作者，当鼠标移动到单元格上是可以在状态栏中看到该内容.
		comment.setAuthor("lion");
